...
def count_papers(resp) -> Tuple[int, int]:
    """Return (retrieved, used) paper counts from a Falcon verbose response."""
    env = getattr(resp, "environment_frame", {}) or {}
    paper_meta = env.get("paper_metadata", [])
    n_retrieved = len(paper_meta)

    used_ids = {
        ctx.get("paper_id")
        for ctx in env.get("contexts", [])
        if ctx.get("is_used")
    }
    n_used = len([p for p in paper_meta if p.get("paper_id") in used_ids])
    return n_retrieved, n_used
...
with open(out_path, "w", encoding="utf-8", newline="") as out:
    ...
    writer.writerow(["protein", "retrieved_papers", "used_papers"])
    for row in rows_to_submit:
        ...
        # run synchronously and request verbose output
        resp = client.run_tasks_until_done(task, verbose=True)
        contexts = resp[0].environment_frame["state"]["state"]["response"]["answer"].get("contexts", [])

        # writer.writerow([row["protein"], n_retrieved, n_used])
        # print(f"Processed {row['protein']} → retrieved={n_retrieved}, used={n_used}")
        time.sleep(0.1)
        break
